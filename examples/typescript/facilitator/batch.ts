...
import { evmBatchSettle } from "x402x-utils";
...
// eslint-disable-next-line no-console
console.log(`EVM Facilitator account (Batch Mode): ${account.address}`);
...
app.post("/settle", async (req, res) => {
  try {
    ...
    if (!batchResponse.success) {
      res.status(500).json({
        error: batchResponse.errorReason || "Settlement failed",
      });
      return;
    }

    // 转换为标准的 SettleResponse 格式
    const response: SettleResponse = {
      success: true,
      transaction: batchResponse.transaction,
      network: batchResponse.network,
    };
...
app.listen(PORT, () => {
  // eslint-disable-next-line no-console
  console.log(`Facilitator (Batch Mode) listening at http://localhost:${PORT}`);
});
